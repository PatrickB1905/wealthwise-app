...
def test_tick_emits_updates(monkeypatch: Any):
    class FakeRepo:
    ...
    def fake_fetch_quotes(symbols: list[str]):
        return [{"symbol": symbols[0], "currentPrice": 1.0, "dailyChangePercent": 0.0}]

    monkeypatch.setattr(poller_mod, "PositionsRepository", FakeRepo)
    monkeypatch.setattr(poller_mod, "fetch_quotes", fake_fetch_quotes)
...
def test_tick_skips_when_no_symbols_or_no_quotes(monkeypatch: Any):
    class FakeRepo:
        def __init__(self, session: Any) -> None:
            self._session = session
...
def test_start_stop_idempotent(monkeypatch: Any):
